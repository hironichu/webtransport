...
try {
  ...
  serverPTR = lib.symbols.proc_server_init( sender.pointer, 4433 ,true, 0, 100, certpath[0], certpath[1], keypath[0], keypath[1]);
  new_connection = new Deno.UnsafeCallback(
	...
	async (client) => {
		...
		lib.symbols.proc_server_init_streams(client, DBufferB, DBufferB.byteLength);
		const StreamBuffer = new ReadableStream({

			async pull(controller) {
				try {
					const nread = await lib.symbols.proc_recv_datagram(client);
					if (nread > 0) {
						controller.enqueue(DBufferB.subarray(0, nread as number));
					}
				} catch (e) {
					controller.error(e);
				}
